...
- (NSArray *)objectsForEntry:(NSDictionary *)theEntry {
		...
		NSPredicate *predicate = [QSMDQueryPredicate predicateWithString:string];
		
//		NSPredicate *predicate = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"kMDItemFinderComment contains \"@\"", [[NSUserDefaults standardUserDefaults] objectForKey:@"QSTagPrefix"]]];
...
//				
	...
	if ([tagQuery resultCount]) {
		...
		foreach(tag, [self tagsFromQuery:tagQuery]) {
			newObject = [QSObject objectWithType:@"qs.tag.file" value:tag name:tag];
